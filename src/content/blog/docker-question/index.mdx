...
| --------- | ------------------------------ | --------------------------------------------------------------------------------------- |
| `PID`     | Process IDs                    | A container sees **only its own** processes. You can’t `ps aux` into other containers.  |
...
| **8. Set Entrypoint & CMD**        | Docker determines the command to run using the image’s `ENTRYPOINT` and `CMD`, or overrides them via the CLI.                                  |
...
| **10. Monitor the Process**        | Docker monitors this process — when it exits, the container stops.                                                                             |
